...
namespace :devto do
  ...
  task :collect, [:org, :since] do |_t, args|
    args.with_defaults(org: ENV.fetch('DEVTO_ORG_SLUG', nil), since: ENV['DEVTO_SINCE'] || '2025-06-01')
  ...
  task :dry_run, [:org, :since] do |_t, args|
    args.with_defaults(org: ENV.fetch('DEVTO_ORG_SLUG', nil), since: ENV['DEVTO_SINCE'] || '2025-06-01')
    raise 'No organization provided. Set DEVTO_ORG_SLUG or pass org argument.' unless args[:org]
